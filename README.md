...
В workflow используется официальный Xray-core, а проверка выполняется реальным HTTPS-запросом к `https://www.youtube.com/generate_204` через локальный SOCKS5-вход каждого узла. Поэтому «рабочий» означает, что узел пропускает запрос к домену YouTube именно из IP-адресов GitHub Actions, а не просто отвечает на ping. Это не гарантирует нормальную скорость воспроизведения видео, но отбрасывает узлы, через которые YouTube недоступен.
