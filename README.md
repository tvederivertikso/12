...
В workflow используется официальный Xray-core, а проверка выполняется запросом к `https://www.gstatic.com/generate_204` через локальный SOCKS5-вход каждого узла. Поэтому «рабочий» означает доступный именно из IP-адресов GitHub Actions, а не обязательно из твоего домашнего интернета.
