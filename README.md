...
В workflow используется официальный Xray-core. Каждый узел проверяется через локальный SOCKS5-вход двумя раундами по трём российским endpoint-ам: `avito.ru`, `vk.com` и `rutube.ru`. Каждый запрос должен вернуть `HTTP 200` и загрузить тело страницы не менее 1000 байт. Узел публикуется только если успешно прошёл минимум 4 из 6 проверок — это отсекает случайные разовые срабатывания и нестабильные прокси. Проверка выполняется из IP-адресов GitHub Actions, а не из твоей домашней сети; это проверка доступности российских платформ, но не полноценная проверка именно из российского IP.

Так как пул вырос до десятков тысяч узлов, ежечасно проверяется не всё: берутся все узлы из предыдущего результата [`sub.txt`](output/sub.txt) (чтобы проверенные узлы не выпадали из подписки) и случайная выборка свежих кандидатов до лимита `--limit` из [`check.yml`](.github/workflows/check.yml) (сейчас 3000). За несколько прогонов весь пул оказывается просмотрен; полный охват одного прогона при 40 воркерах занял бы часы и упёрся бы в лимиты GitHub Actions.
